var geoLocationCache = "Majors";

$(function () {

    $(".button.whiteButton:contains('Majors')").click(function (e) {
        e.preventDefault();
        geoLocationCache = "Majors";
        refreshCurrencyGrid("Majors");
    });
    $(".button.whiteButton:contains('EU')").click(function (e) {
        e.preventDefault();
        geoLocationCache = "EU";
        refreshCurrencyGrid("EU");
    });
    $(".button.whiteButton:contains('Americas')").click(function (e) {
        e.preventDefault();
        geoLocationCache = "Americas";
        refreshCurrencyGrid("Americas");
    });
    $(".button.whiteButton:contains('Asia')").click(function (e) {
        e.preventDefault();
        geoLocationCache = "Asia";
        refreshCurrencyGrid("Asia");
    });
    $(".button.whiteButton:contains('Africa')").click(function (e) {
        e.preventDefault();
        geoLocationCache = "Africa";
        refreshCurrencyGrid("Africa");
    });
    $(".button.whiteButton:contains('Middle East')").click(function (e) {
        e.preventDefault();
        geoLocationCache = "Middle East";
        refreshCurrencyGrid("Middle East");
    });
...
var CurrencyList = [
	...
	{"Name":"AED","Description":"United Arab Emirates Dirham","IsMajor":false,"Geography":"Middle East"},
	...
	{"Name":"ILS","Description":"Israel Shekel","IsMajor":false,"Geography":"Middle East"},
	{"Name":"MXN","Description":"Mexican Peso","IsMajor":false,"Geography":"Americas"},
	{"Name":"BHD","Description":"Bahraini Dinar","IsMajor":false,"Geography":"Middle East"},
...
function getRatesNestedArray(geoLocation, data) {
    var allRates = _.pairs(data.rates);
    var activeCurrencies = _.map(
        _.filter(CurrencyList, function (curr) {
            if (!geoLocation) {
                return true;
            } else {
                return geoLocation === 'Majors' ? curr.IsMajor : curr.Geography === geoLocation;
            }
        }), function (c) { return c.Name; });

    var filtered = _.filter(allRates, function (rateArr) {
        return _.intersection([rateArr[0]], activeCurrencies).length !== 0;
    });
...
function refreshCurrencyGrid(geoLocation) {
    if (_.intersection([geoLocation], ['Majors', 'EU', 'Americas', 'Asia', 'Africa', 'Middle East']).length === 1) {
        $("#jsGrid").empty();
                    ...
                    if (!tmpRates || isNaN(tmpRates.timestamp) ||
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                deferred.resolve(_.first(getRatesNestedArray(geoLocation, data), 15));
                            }
                        });
                    } else {
                        deferred.resolve(_.first(getRatesNestedArray(geoLocation, tmpRates), 15));
                    }
...
function currencyWidget(){	// currency widget -----------------------------------------------
	...
	$("#amount").bind('input', function(e){
	    ...
	    convertAndSetAmountCounted(text);
		text = text.replace(/ /g,"");
